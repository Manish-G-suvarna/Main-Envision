...
var eventRoutes = {
  ...
  async getEvents(env2) {
    try {
      ...
      console.error("Error fetching events:", error3);
      return new Response(
        JSON.stringify({
          success: false,
  ...
  async getEventsByDepartment(env2, departmentId) {
    try {
      const { results } = await env2.DB.prepare(`
        SELECT
          e.id,
      ...
      console.error("Error fetching events by department:", error3);
      return new Response(
        JSON.stringify({
          success: false,
  ...
  async getMegaEvents(env2) {
    try {
      ...
      console.error("Error fetching mega events:", error3);
      return new Response(
        JSON.stringify({
          success: false,
...
var teamRoutes = {
  ...
  async getCoreTeam(env2) {
    try {
      ...
      `).all();
      const groupedByTeam = (results || []).reduce((acc, member) => {
      ...
      console.error("Error fetching core team:", error3);
      return new Response(
        JSON.stringify({
          success: false,
  ...
  async getTeamById(env2, teamId) {
    try {
      ...
      console.error("Error fetching team by ID:", error3);
      return new Response(
        JSON.stringify({
          success: false,
  ...
  async fetch(request, env2) {
    ...
    try {
      ...
      if (pathname === "/api/health") {
        ...
        response = await eventRoutes.getEvents(env2);
      } else if (pathname.startsWith("/api/events/")) {
        const departmentId = pathname.split("/")[3];
        response = await eventRoutes.getEventsByDepartment(env2, departmentId);
      } else if (pathname === "/api/team") {
        response = await teamRoutes.getTeams(env2);
      } else if (pathname === "/api/team/core") {
        response = await teamRoutes.getCoreTeam(env2);
      } else {
        response = new Response(
...
// .wrangler/tmp/bundle-cPbLDi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cPbLDi/middleware-loader.entry.ts
